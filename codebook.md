...
1. The `cites` field returned by the CourtListener API for each opinion (209 edges)
2. Full-text citation extraction using the eyecite library applied to downloaded opinion text (114 additional edges)

The CourtListener cites field coverage rate — the fraction of Eyecite-extracted edges already present in the CourtListener cites field — was 65.0%. This is an overlap ratio between two extraction methods, not a recall rate against a ground truth. No ground truth edge set exists for this corpus.
...
The citation network in GEXF format with full node attributes embedded directly
in the graph file. Identical graph structure to `s230_graph.gexf` (63 nodes,
320 edges, confirmed DAG) but each node carries additional attributes:
`pagerank`, `community`, `in_degree`, `case_name`, `year`, `court`, `cite_count`,
`citation`, and `url`. Suitable for direct import into Gephi with all metrics
pre-loaded. Generated by `03_compute_metrics.py`.
